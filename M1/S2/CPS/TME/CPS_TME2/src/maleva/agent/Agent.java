...
import maleva.framework.BasicComposite;
import maleva.framework.LCInitializeController;
import maleva.framework.LifeCycleException;
import maleva.framework.ServiceBindException;

public abstract class Agent extends BasicComposite 
implements /* requires */
RequireEnvironmentFetchService,
RequireEnvironmentRegisterService,
/* controllers */
LCInitializeController, 
LCConfigureAgentController,
LCStepperController {
	...
	/* external bindings */
	public abstract void bindEnvironmentFetchService(EnvironmentFetchService efs) throws ServiceBindException;
	
	public void bindEnvironmentRegisterService(EnvironmentRegisterService ers) throws ServiceBindException {
		this.ers = ers;
	}
	
	public abstract void bindEnvironmentEffectService(EnvironmentEffectService ees) throws ServiceBindException;
	...
	public void initialize() throws LifeCycleException {
		if(initialized)
			throw new LifeCycleException("Cannot initialize : agent already initialized");
		initialized = true;
	}
	
	public void configure(Environnement env, int x, int y) throws LifeCycleException {
		if(!initialized)
			throw new LifeCycleException("Cannot configure : agent not initialized");
		if(configured)
			throw new LifeCycleException("Cannot configure : agent already configured");
